...
from processamento import (
    formatar_br, parse_float_inteligente, parse_data_br,
    detecting_category, detectar_categoria_plenus, sort_key_nomes,
    transform_data_sistransf, to_excel_autoajustado,
    limpar_para_comparacao, calcular_similaridade_avancada, gerar_sugestao_nome_primeiro,
    extrair_dados_sisflora, load_data_consumo_excel, extrair_dados_plenus_html,
    MAPA_CORRECAO_PRODUTOS, CODIGOS_ACEITOS, TERMOS_GENERICOS, COLS_SISTRANSF_EXCEL
)
from database import (
    init_db, get_db_connection, excluir_periodo_tabela, load_app_state, save_app_state,
    carregar_agrupamentos_db, carregar_todos_agrupamentos_db, get_categorias_dos_grupos,
    salvar_agrupamento_db, excluir_grupo_db, carregar_vinculos_db, carregar_lista_grupos_db,
    salvar_vinculo_db, excluir_vinculo_db, check_dates_exist, salvar_lote_smart,
    carregar_transf_filtrado_db, get_valores_unicos_coluna, carregar_plenus_movimento_db,
    carregar_consumo_filtrado_db, salvar_lote_sisflora_db, carregar_sisflora_data_db,
    get_datas_sisflora_disponiveis, excluir_sisflora_por_data,
    get_max_date_db, get_smart_date_range, update_session_dates
)
from ui import (
    render_filtered_table, render_plenus_dashboard
)

# --- CONFIGURAÇÃO ---
st.set_page_config(page_title="🌲 Sistema S&P - Conferência v186", layout="wide")
...
st.title("🌲 CONFERENCIA ESTOQUE v186")
    ...
    if op_ple == "Ler HTML / Importar":
        ...
        if 'df_plenus' in st.session_state and not st.session_state['df_plenus'].empty and st.session_state.get('ple_source') == 'upload':
            ...
            if st.session_state.get('lista_erro_plenus'):
                 with st.expander("⚠️ Ver Erros de Leitura (Sem Total) - Estes itens não aparecem no saldo oficial", expanded=False):
                     st.dataframe(pd.DataFrame(st.session_state['lista_erro_plenus']))

            # REFACTOR V183: Use shared dashboard function
            # Importante: dados_do_html=True para não aplicar filtros de data e não remover duplicatas
            render_plenus_dashboard(st.session_state['df_plenus'], key_prefix="upload", allow_save=True, dados_do_html=True)
        ...
        if st.button("Carregar do Histórico", key="btn_load_hist_p"):
            ...
            if not df_hist.empty:
                # Adaptação de colunas para formato esperado
        ...
        if 'df_plenus' in st.session_state and not st.session_state['df_plenus'].empty and st.session_state.get('ple_source') == 'history':
            ...
            # Importante: dados_do_html=False para aplicar filtros de data e lógica de histórico
            render_plenus_dashboard(st.session_state['df_plenus'], key_prefix="history", allow_save=False, dados_do_html=False)
    ...
    with tab_import:
        ...
        if st.session_state['st_df_transf_preview'] is not None:
            ...
            if st.button("💾 Salvar Transformações no Banco", key="btn_save_transf"):
                ...
                if ins > 0: st.success(f"✅ {ins} registros salvos.")
                if ext > 0: st.warning(f"⚠️ {ext} registros ignorados (já existiam).")
    ...
    if admin_mode == "Agrupar Sisflora":
        ...
        with st.expander("🤖 Sugestões Inteligentes de Vínculo", expanded=True):
            ...
            if st.button("🔎 Buscar Sugestões"):
                with st.spinner(f"Analisando essência dos nomes ({f_cat_ia})..."):
                    ...
                    for gp in pendentes_ple:
                        ...
                        if is_vinculado: status_vinc = f"✅ JÁ VINCULADO A: {vinculos_atuais[gp]}"
            ...
            if 'sugestoes_ia' in st.session_state and st.session_state['sugestoes_ia']:
                ...
                else: st.info(f"Nenhuma sugestão encontrada para o filtro {f_cat_ia}.")
