...
# --- CONFIGURAÇÃO ---
st.set_page_config(page_title="🌲 Sistema S&P - Conferência v192", layout="wide")
...
with st.sidebar.expander("📋 Notas de Versão v192", expanded=False):
    st.markdown("### 🆕 O que melhorou na v192:")
    for melhoria in CHANGELOG["v192"]["melhorias"]:
    ...
    if mostrar_como_ler:
        ...
        **📝 Estrutura:**
        - Versão atual (v192) é exibida expandida por padrão
    ...
    for versao in sorted(CHANGELOG.keys(), reverse=True)[1:]:  # Pula a v192 (já mostrada)
...
if menu_sel == "1. SALDO SISFLORA":
    st.title("🌲 CONFERENCIA ESTOQUE v192 - SALDO SISFLORA")
elif menu_sel == "2. SALDO PLENUS":
    st.title("🌲 CONFERENCIA ESTOQUE v192 - SALDO PLENUS")
    # Nota de versão expandível no topo
    with st.expander("📋 Notas da Versão v192 - Clique para ver o que melhorou", expanded=False):
        st.markdown("### 🆕 Principais melhorias na v192:")
        for melhoria in CHANGELOG["v192"]["melhorias"]:
            st.markdown(f"{melhoria}")
        ...
        if mostrar_como_ler_topo:
            ...
            **💡 Dica:** Consulte também a sidebar para ver versões anteriores completas.
            """)
    st.caption("✅ v192: Definição automática de datas ao carregar do histórico, exibição de quantidade de SKUs, correções de ordenação e duplicatas")
elif menu_sel == "3. HISTORICO TRANSFORMAÇÃO":
    st.title("🌲 CONFERENCIA ESTOQUE v192 - HISTÓRICO TRANSFORMAÇÃO")
elif menu_sel == "4. DEBITO CONSUMO":
    st.title("🌲 CONFERENCIA ESTOQUE v192 - DÉBITO CONSUMO")
elif menu_sel == "5. Gestão: Vínculos (Admin)":
    st.title("🌲 CONFERENCIA ESTOQUE v192 - GESTÃO DE VÍNCULOS")
elif menu_sel == "6. Conferência & Auditoria":
    st.title("🌲 CONFERENCIA ESTOQUE v192 - CONFERÊNCIA & AUDITORIA")
else:
    st.title("🌲 CONFERENCIA ESTOQUE v192")
...
# --- 2. SALDO PLENUS (UPDATED v183) ---
    ...
    if op_ple == "Ler HTML / Importar":
        ...
        if 'df_plenus' in st.session_state and not st.session_state['df_plenus'].empty and st.session_state.get('ple_source') == 'history':
            ...
            if 'Cat_Auto' in df_ple_mem.columns:
                categorias_disponiveis = sorted(df_ple_mem['Cat_Auto'].dropna().unique())
                if categorias_disponiveis:
                    cat_selecionadas = st.multiselect(
                        "Filtrar por Categoria (Cat_Auto):",
                        options=["TODOS"] + categorias_disponiveis,
                        default=["TODOS"],
                        key="filtro_cat_ple_db"
                    )

                    # Aplica filtro de categoria
                    # IMPORTANTE: Filtrar por Cat_Auto, mas manter linhas TOTAL/ANTERIOR dos SKUs filtrados
                    if cat_selecionadas:
                        if "TODOS" in cat_selecionadas:
                            pass  # Mantém df_ple_mem sem filtrar
                        else:
                            cat_para_filtrar = [c for c in cat_selecionadas if c != "TODOS"]
                            if cat_para_filtrar:
                                # SIMPLIFICAÇÃO: Filtrar diretamente por Cat_Auto em TODAS as linhas
                                # Isso garante que produtos com TOTAL/ANTERIOR sejam incluídos se pertencerem à categoria
                                if 'Cat_Auto' in df_ple_mem.columns:
                                    # Filtrar TODAS as linhas (movimentações, TOTAL, ANTERIOR) por categoria
                                    df_ple_mem = df_ple_mem[df_ple_mem['Cat_Auto'].isin(cat_para_filtrar)].copy()
                                else:
                                    # Se não tem Cat_Auto, tentar usar categoria
                                    if 'categoria' in df_ple_mem.columns:
                                        df_ple_mem = df_ple_mem[df_ple_mem['categoria'].isin(cat_para_filtrar)].copy()
