...
import json
import os
from processamento import formatar_br, parse_float_inteligente
...
def render_plenus_dashboard(df_full, key_prefix="p_dash", allow_save=True, dados_do_html=False):
    # --- Date Filter ---
    # Determine bounds
    ...
    # CORREÇÃO: Se dados vêm do HTML, NÃO aplicar filtro de data (HTML já tem todas as linhas corretas)
    df_view = df_full.copy()
    if not dados_do_html and 'data_movimento' in df_view.columns:
    ...
    # CORREÇÃO v193: Ordem após SALDO: Vlr Unitário, Total Vlr, Nota, Série
    cols_table = ['data', 'sku', 'produto', 'categoria', 'tipo', 'saldo_anterior', 'entrada', 'saida', 'saldo', 'vlr_unit', 'vlr_total', 'nota', 'serie']
    ...
    # V193: Garantir que vlr_unit e vlr_total sempre existem (criar com 0 se não existirem)
    if 'vlr_unit' not in df_view.columns:
        df_view['vlr_unit'] = 0.0
    else:
        df_view['vlr_unit'] = df_view['vlr_unit'].fillna(0.0)

    if 'vlr_total' not in df_view.columns:
        df_view['vlr_total'] = 0.0
    else:
        df_view['vlr_total'] = df_view['vlr_total'].fillna(0.0)
    ...
    # CORREÇÃO CRÍTICA: Se dados_do_html=True, NÃO recalcular saldos das linhas TOTAL (usar valores originais do HTML)
    ...
    # CORREÇÃO CRÍTICA: Se dados_do_html=True, NÃO criar linhas TOTAL automaticamente (HTML já tem todas as linhas TOTAL)

    if 'saldo' in df_view.columns and not df_view.empty and 'sku' in df_view.columns and not dados_do_html:
                    ...
                    if not df_calc_totais_sem_total.empty:
                        ...
                        for idx, row in ultimas_linhas.iterrows():
                            ...
                            for col in df_calc_totais_sem_total.columns:
                                # CORREÇÃO: Não copiar 'id' do banco para linhas TOTAL criadas dinamicamente
                                # Isso permite identificar linhas originais vs criadas dinamicamente
                                if col == 'id':
                                    linha_total[col] = None  # Linhas criadas dinamicamente não têm id do banco
                                else:
        ...
        # CORREÇÃO CRÍTICA: Se dados vêm do HTML, NÃO remover duplicatas (HTML já tem todas as linhas corretas)
        if not dados_do_html and 'tipo' in df_view.columns and 'sku' in df_view.columns:
            mask_total = df_view['tipo'].astype(str).str.strip().str.upper().isin(['TOTAL', 'TOTAL:', 'TOTAL '])
            if mask_total.any():
    ...
    # CORREÇÃO CRÍTICA: Na v183, o filtro de categoria é aplicado ANTES do cálculo do total
    # v183 linha 1134-1138: f_cat = c1.multiselect("Categoria:", cats, key="fp_cat")
    # if f_cat: df_view = df_view[df_view['categoria'].isin(f_cat)]
    # vol_total = df_view[df_view['tipo'].isin(['Total', 'TOTAL'])]['saldo'].sum()
    # IMPORTANTE: Adicionar filtro de categoria na UI ANTES do cálculo (como na v183)
    # O usuário deve poder escolher qual categoria filtrar (TORAS, SERRADAS, BENEFICIADAS, etc)
    # Filtro de categoria (igual v183) - ANTES do cálculo
    f_cat_aplicado = None
    if 'Cat_Auto' in df_view.columns or 'categoria' in df_view.columns:
        # Usar Cat_Auto se disponível, senão usar categoria
        col_cat = 'Cat_Auto' if 'Cat_Auto' in df_view.columns else 'categoria'
        categorias_disponiveis = sorted(df_view[col_cat].dropna().unique())

        if categorias_disponiveis:
            # Debug: verificar categorias antes do filtro
            if 'tipo' in df_view.columns:
                mask_total_debug = df_view['tipo'].astype(str).str.strip().str.upper().isin(['TOTAL', 'TOTAL:', 'TOTAL ', 'TOTAL'])
                df_totais_debug = df_view[mask_total_debug].copy()
                if 'Cat_Auto' in df_totais_debug.columns:
                    cats_antes = df_totais_debug['Cat_Auto'].value_counts().to_dict()
                    st.caption(f"🔍 DEBUG ANTES FILTRO: {len(df_totais_debug)} linhas TOTAL. Categorias: {cats_antes}")

            # Filtro de categoria (igual v183)
            # CORREÇÃO: Usar session_state para garantir que o filtro seja aplicado corretamente
            filtro_cat_key = f"{key_prefix}_filtro_cat_state"
            if filtro_cat_key not in st.session_state:
                st.session_state[filtro_cat_key] = []

            f_cat = st.multiselect("Filtrar por Categoria:", categorias_disponiveis, key=f"{key_prefix}_filtro_cat", default=st.session_state.get(filtro_cat_key, []))

            # Atualizar session_state quando o filtro mudar
            if f_cat != st.session_state.get(filtro_cat_key, []):
                st.session_state[filtro_cat_key] = f_cat
                # Forçar rerun para aplicar o filtro e recalcular o total
                st.rerun()

            # Aplicar filtro de categoria (igual v183)
            # IMPORTANTE: Se não houver seleção, mostrar todas as categorias (comportamento padrão)
            # Mas se o usuário selecionar, aplicar o filtro
            if f_cat:
                # CORREÇÃO CRÍTICA: Filtrar por SKU, não por linha individual
                # O problema é que algumas linhas TOTAL/ANTERIOR podem não ter a categoria preenchida corretamente
                # A solução é identificar quais SKUs pertencem à categoria selecionada (baseado nas movimentações)
                # e incluir TODAS as linhas (TOTAL, ANTERIOR e movimentações) desses SKUs

                # Passo 1: Identificar SKUs que pertencem à categoria selecionada
                # IMPORTANTE: Usar df_full original (não df_view) para identificar os SKUs, pois df_view já pode estar filtrado
                # Isso garante que identificamos TODOS os SKUs da categoria, mesmo que algumas linhas não tenham categoria preenchida
                if 'tipo' in df_full.columns and 'sku' in df_full.columns:
                    # Usar a mesma coluna de categoria que estamos usando no filtro
                    col_cat_full = 'Cat_Auto' if 'Cat_Auto' in df_full.columns else 'categoria'
                    skus_da_categoria = set()

                    # Identificar SKUs baseado em TODAS as linhas (movimentações, TOTAL e ANTERIOR) que têm a categoria correta
                    # Isso garante que capturamos todos os SKUs da categoria, mesmo que apenas algumas linhas tenham a categoria
                    mask_cat_full = df_full[col_cat_full].isin(f_cat)
                    skus_da_categoria.update(df_full[mask_cat_full]['sku'].dropna().unique())

                    # DEBUG: Verificar quantos SKUs foram identificados
                    st.caption(f"🔍 DEBUG FILTRO: Identificados {len(skus_da_categoria)} SKUs da categoria {', '.join(f_cat)}")

                    # Passo 2: Filtrar df_view para incluir TODAS as linhas dos SKUs identificados
                    # Isso garante que linhas TOTAL/ANTERIOR sejam incluídas mesmo se não tiverem categoria preenchida
                    mask_sku_categoria = df_view['sku'].isin(skus_da_categoria)

                    # Também incluir linhas que já têm a categoria correta diretamente (para garantir)
                    mask_cat_direto = df_view[col_cat].isin(f_cat)

                    # Combinar ambos os filtros: SKU da categoria OU categoria direta
                    mask_cat_final = mask_sku_categoria | mask_cat_direto

                    # Também incluir linhas onde a categoria é None se "None" estiver na seleção
                    tem_none = None in f_cat
                    tem_na = False
                    try:
                        tem_na = any(pd.isna(x) for x in f_cat)
                    except (TypeError, ValueError):
                        tem_na = any(x is pd.NA or (isinstance(x, float) and pd.isna(x)) for x in f_cat)
                    if tem_none or tem_na:
                        mask_cat_final = mask_cat_final | df_view[col_cat].isna()

                    df_view = df_view[mask_cat_final].copy()
                else:
                    # Fallback: se não tem tipo ou sku, usar filtro direto por categoria
                    mask_cat = df_view[col_cat].isin(f_cat)
                    tem_none = None in f_cat
                    tem_na = False
                    try:
                        tem_na = any(pd.isna(x) for x in f_cat)
                        ...
                        tem_na = any(x is pd.NA or (isinstance(x, float) and pd.isna(x)) for x in f_cat)
                    if tem_none or tem_na:
                        mask_cat = mask_cat | df_view[col_cat].isna()
                    df_view = df_view[mask_cat].copy()

                f_cat_aplicado = f_cat

                # Debug: verificar categorias depois do filtro
                if 'tipo' in df_view.columns:
                    mask_total_debug2 = df_view['tipo'].astype(str).str.strip().str.upper().isin(['TOTAL', 'TOTAL:', 'TOTAL ', 'TOTAL'])
                    df_totais_debug2 = df_view[mask_total_debug2].copy()
                    if 'Cat_Auto' in df_totais_debug2.columns:
                        cats_depois = df_totais_debug2['Cat_Auto'].value_counts().to_dict()
                        skus_totais = len(df_totais_debug2['sku'].dropna().unique()) if 'sku' in df_totais_debug2.columns else 0
                        st.caption(f"🔍 DEBUG DEPOIS FILTRO: {len(df_totais_debug2)} linhas TOTAL ({skus_totais} SKUs únicos). Categorias: {cats_depois}")
                        # CORREÇÃO CRÍTICA: Verificar se há linhas TOTAL de categorias diferentes
                        if len(cats_depois) > len(f_cat):
                            st.warning(f"⚠️ ATENÇÃO: Encontradas linhas TOTAL de {len(cats_depois)} categorias diferentes após filtrar por {len(f_cat)} categoria(s). Isso pode indicar problema no filtro.")

                # Debug temporário
                st.caption(f"✅ Filtro aplicado: {len(f_cat)} categoria(s) selecionada(s): {', '.join(f_cat)}")
            else:
                st.caption(f"ℹ️ Nenhuma categoria selecionada - mostrando todas as categorias ({len(categorias_disponiveis)} categorias)")

    # CORREÇÃO CRÍTICA v194: Calcular Saldo Total DEPOIS de todos os filtros (categoria e busca)
    # Isso garante que o saldo reflita apenas os produtos visíveis após os filtros
    # Calcular Saldo Total ANTES da tabela - método correto: somar APENAS saldos das linhas tipo TOTAL
    # CORREÇÃO CRÍTICA: Usar a mesma lógica da v183 que funciona corretamente
    # v183: vol_total = df_view[df_view['tipo'].isin(['Total', 'TOTAL'])]['saldo'].sum()
    # IMPORTANTE: Na v183, o cálculo é feito DEPOIS do filtro de categoria ser aplicado
    # Por isso, precisamos garantir que o cálculo use apenas as linhas que estão visíveis após os filtros
    # ============================================================================
    # REGRA CRÍTICA PARA CÁLCULO DE SALDO TOTAL - LER SEMPRE ANTES DE MODIFICAR
    # ============================================================================
    # O saldo total DEVE ser calculado da seguinte forma:
    # 1. Filtrar APENAS linhas com tipo='TOTAL' ou 'TOTAL:'
    # 2. Para histórico: remover duplicatas por SKU (manter apenas uma linha TOTAL por SKU)
    # 3. Para HTML: NÃO remover duplicatas (HTML já tem todas as linhas corretas)
    # 4. Somar APENAS o campo 'saldo' das linhas TOTAL filtradas
    # 5. NÃO somar linhas ANTERIOR, NÃO somar movimentações, APENAS linhas TOTAL
    # 6. Se houver filtros (categoria/busca), aplicar os filtros ANTES de filtrar por tipo TOTAL
    # 7. A MESMA LÓGICA deve ser usada tanto para saldo geral quanto para saldo com filtros
    # ============================================================================
    # CORREÇÃO FINAL: Criar função auxiliar para calcular total de forma consistente
    def calcular_total_saldo(df_calc, remover_duplicatas=True):
        """
        Calcula o total somando APENAS linhas TOTAL (tipo='TOTAL' ou 'TOTAL:')

        ⚠️ REGRA CRÍTICA - NÃO SOMAR LINHAS ANTERIOR:
        - Filtrar APENAS linhas tipo TOTAL (exclui ANTERIOR e movimentações)
        - Somar APENAS o campo 'saldo' das linhas TOTAL filtradas
        - NÃO somar linhas ANTERIOR (nunca)
        - NÃO somar movimentações (nunca)

        Parâmetros:
        - df_calc: DataFrame já filtrado por categoria/busca (pode conter TOTAL, ANTERIOR e movimentações)
        - remover_duplicatas: Se True, remove duplicatas por SKU (histórico)
                            Se False, não remove (HTML já tem todas corretas)

        Retorna:
        - total: Soma dos saldos das linhas TOTAL
        - df_totais: DataFrame contendo apenas as linhas TOTAL usadas no cálculo
        """
        total = 0.0
        df_totais = pd.DataFrame()
        if 'saldo' in df_calc.columns and 'tipo' in df_calc.columns and not df_calc.empty:
            # REGRA 1: Filtrar APENAS linhas tipo TOTAL (exclui ANTERIOR e movimentações)
            mask_total = df_calc['tipo'].astype(str).str.strip().str.upper().isin(['TOTAL', 'TOTAL:', 'TOTAL ', 'TOTAL'])
            df_totais = df_calc[mask_total].copy()

            # REGRA 2: Para histórico, remover duplicatas por SKU (manter apenas uma linha TOTAL por SKU)
            # REGRA 3: Para HTML, NÃO remover duplicatas (HTML já tem todas as linhas corretas)
            if remover_duplicatas and 'sku' in df_totais.columns and not df_totais.empty:
                df_totais = df_totais.drop_duplicates(subset=['sku'], keep='last')

            # REGRA 4: Somar APENAS o campo 'saldo' das linhas TOTAL (nunca somar ANTERIOR)
            if not df_totais.empty and 'saldo' in df_totais.columns:
                # CORREÇÃO CRÍTICA: Usar parse_float_inteligente para converter corretamente números no formato brasileiro
                # Isso garante que valores como "13.669,272" sejam convertidos corretamente para 13669.272
                df_totais['saldo'] = df_totais['saldo'].apply(parse_float_inteligente)
                # IMPORTANTE: Soma APENAS linhas TOTAL (linhas ANTERIOR já foram excluídas pelo filtro acima)
                total = float(df_totais['saldo'].sum())
        return total, df_totais

    # ============================================================================
    # CÁLCULO DO SALDO TOTAL - APLICAR REGRAS CRÍTICAS ACIMA
    # ============================================================================
    # IMPORTANTE: A MESMA LÓGICA deve ser usada tanto para saldo geral quanto para saldo com filtros
    # A única diferença é o DataFrame de entrada:
    # - Sem filtros: usar df_full original
    # - Com filtros: usar df_view que já foi filtrado por categoria/busca
    # ============================================================================
    # CORREÇÃO CRÍTICA: Verificar se há filtros aplicados (categoria ou busca)
    # txt_search já vem do session_state (linha 177), então está sempre atualizado
    tem_filtros = f_cat_aplicado is not None or (txt_search and str(txt_search).strip())

    if dados_do_html:
        # CORREÇÃO CRÍTICA: Para HTML, SEMPRE usar remover_duplicatas=False (HTML já tem todas corretas)
        # Aplicar a MESMA lógica tanto com filtros quanto sem filtros
        if tem_filtros:
            # REGRA 6: Se houver filtros, aplicar os filtros ANTES de filtrar por tipo TOTAL
            # df_view já está filtrado por categoria/busca, então usar diretamente
            # IMPORTANTE: Usar remover_duplicatas=False (HTML não tem duplicatas, não remover)
            vol_total, df_totais_vol = calcular_total_saldo(df_view, remover_duplicatas=False)
        else:
            # Sem filtros: usar df_full original (lógica que funciona corretamente)
            # IMPORTANTE: Usar remover_duplicatas=False (HTML não tem duplicatas, não remover)
            vol_total, df_totais_vol = calcular_total_saldo(df_full, remover_duplicatas=False)
        total_saldo = vol_total
        df_totais_calc = df_totais_vol
    else:
        # Para histórico: sempre filtrar linhas TOTAL pelo período selecionado
        # CORREÇÃO CRÍTICA v194: Se os dados vieram do HTML (identificado por arquivo_origem), NÃO remover duplicatas
        # O banco deve ser o espelho do HTML, então se veio do HTML, usar mesma lógica (sem remover duplicatas)
        veio_do_html = False
        if 'arquivo_origem' in df_full.columns and not df_full.empty:
            # Se tem arquivo_origem preenchido, significa que veio do HTML
            arquivos_origem = df_full['arquivo_origem'].dropna().unique()
            if len(arquivos_origem) > 0:
                veio_do_html = True

        if tem_filtros:
            # REGRA 6: Se houver filtros, usar df_view que JÁ está filtrado por categoria/busca
            # CORREÇÃO CRÍTICA: df_view já foi filtrado por categoria e busca, então usar diretamente
            # NÃO filtrar novamente por período, pois isso pode excluir linhas TOTAL que devem ser incluídas
            # As linhas TOTAL devem ser incluídas se o SKU está visível após os filtros
            df_view_para_calculo = df_view.copy()
            # IMPORTANTE: Se veio do HTML, NÃO remover duplicatas (mesma lógica do HTML)
            # Se não veio do HTML, remover duplicatas (banco pode ter duplicatas de importações antigas)
            remover_dup = not veio_do_html
            vol_total, df_totais_vol = calcular_total_saldo(df_view_para_calculo, remover_duplicatas=remover_dup)
        else:
            # Sem filtros: usar df_full original, filtrar apenas por período
            df_full_para_calculo = df_full.copy()
            if 'data_movimento' in df_full_para_calculo.columns and 'tipo' in df_full_para_calculo.columns:
                # Filtrar linhas TOTAL pelo período selecionado
                df_full_para_calculo['dt_temp'] = pd.to_datetime(df_full_para_calculo['data_movimento'], errors='coerce').dt.date
                mask_total = df_full_para_calculo['tipo'].astype(str).str.strip().str.upper().isin(['TOTAL', 'TOTAL:', 'TOTAL ', 'TOTAL'])
                # Filtrar apenas linhas TOTAL dentro do período selecionado
                mask_total_no_periodo = mask_total & ((df_full_para_calculo['dt_temp'] >= d_ini_f) & (df_full_para_calculo['dt_temp'] <= d_fim_f))
                df_full_para_calculo = df_full_para_calculo[mask_total_no_periodo].copy()
                df_full_para_calculo = df_full_para_calculo.drop(columns=['dt_temp'])
            else:
                # Se não tem data_movimento, usar todas as linhas TOTAL
                mask_total = df_full_para_calculo['tipo'].astype(str).str.strip().str.upper().isin(['TOTAL', 'TOTAL:', 'TOTAL ', 'TOTAL'])
                df_full_para_calculo = df_full_para_calculo[mask_total].copy()
            # IMPORTANTE: Se veio do HTML, NÃO remover duplicatas (mesma lógica do HTML)
            # Se não veio do HTML, remover duplicatas (banco pode ter duplicatas de importações antigas)
            remover_dup = not veio_do_html
            vol_total, df_totais_vol = calcular_total_saldo(df_full_para_calculo, remover_duplicatas=remover_dup)
        total_saldo = vol_total
        df_totais_calc = df_totais_vol

    # Debug: verificar quantas linhas TOTAL e categorias
    if not df_totais_vol.empty:
        if 'Cat_Auto' in df_totais_vol.columns:
            cats_totais = df_totais_vol['Cat_Auto'].value_counts().to_dict()
            if dados_do_html:
                st.caption(f"🔍 DEBUG: {len(df_totais_vol)} linhas TOTAL (HTML - sem remover duplicatas). Categorias: {cats_totais}")
            else:
                st.caption(f"🔍 DEBUG: {len(df_totais_vol)} linhas TOTAL (após remover duplicatas). Categorias: {cats_totais}")
        st.caption(f"🔍 DEBUG: Total calculado (vol_total): {formatar_br(vol_total)}")
        if dados_do_html:
            st.caption(f"🔍 DEBUG: Esperado: 18.200,8888 | Diferença: {formatar_br(abs(vol_total - 18200.8888))}")

    # Exibe o total ANTES da tabela
    # Calcular quantidade de SKUs únicos listados
    # CORREÇÃO: Usar df_view que já está filtrado por categoria/busca
    qtd_skus = 0
    if 'sku' in df_view.columns and not df_view.empty:
        # Pegar apenas SKUs únicos das linhas visíveis (excluindo linhas TOTAL/ANTERIOR duplicadas)
        # Mas incluir linhas TOTAL/ANTERIOR para contar o SKU
        qtd_skus = len(df_view['sku'].dropna().unique())

    # CORREÇÃO CRÍTICA: Recalcular vol_total do df_view filtrado para garantir que está correto
    # Isso garante que a métrica mostre o saldo correto quando há filtros aplicados
    # IMPORTANTE: Sempre recalcular quando há filtros, independente de ser HTML ou histórico
    if tem_filtros:
        # Recalcular do df_view que já está filtrado por categoria/busca
        # CORREÇÃO: Identificar se veio do HTML para usar a lógica correta
        veio_do_html_recalc = False
        if 'arquivo_origem' in df_view.columns and not df_view.empty:
            arquivos_origem_recalc = df_view['arquivo_origem'].dropna().unique()
            if len(arquivos_origem_recalc) > 0:
                veio_do_html_recalc = True
        remover_dup_recalc = not veio_do_html_recalc
        # CORREÇÃO CRÍTICA: Usar df_view que já está filtrado por categoria/busca
        # Isso garante que apenas linhas TOTAL dos SKUs visíveis sejam somadas
        vol_total_recalc, df_totais_vol_recalc = calcular_total_saldo(df_view, remover_duplicatas=remover_dup_recalc)

        # DEBUG DETALHADO: Gerar log para análise
        if f_cat_aplicado and not df_totais_vol_recalc.empty:
            log_debug_cat = {
                "timestamp": datetime.now().isoformat(),
                "categoria_filtrada": list(f_cat_aplicado),
                "total_linhas_total": len(df_totais_vol_recalc),
                "total_saldo_calculado": float(vol_total_recalc),
                "total_saldo_formatado": formatar_br(vol_total_recalc),
                "remover_duplicatas": remover_dup_recalc,
                "veio_do_html": veio_do_html_recalc,
                "detalhes_linhas_total": []
            }

            # Adicionar detalhes de cada linha TOTAL
            for idx, row in df_totais_vol_recalc.iterrows():
                log_debug_cat["detalhes_linhas_total"].append({
                    "sku": str(row.get('sku', '')),
                    "produto": str(row.get('produto', '')),
                    "categoria": str(row.get('Cat_Auto', row.get('categoria', ''))),
            ...
            if not os.path.exists(log_dir):
                os.makedirs(log_dir)
            log_filename = os.path.join(log_dir, f"debug_categoria_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
            with open(log_filename, 'w', encoding='utf-8') as f:
                json.dump(log_debug_cat, f, indent=2, ensure_ascii=False)

            st.caption(f"🔍 DEBUG: Log detalhado salvo em: `{log_filename}`")
            st.caption(f"🔍 DEBUG: {len(df_totais_vol_recalc)} linhas TOTAL encontradas. Soma: {formatar_br(vol_total_recalc)}")

            # CORREÇÃO: Verificar se há SKUs que podem estar causando diferença
            # SKUs problemáticos identificados na comparação: 001027, 001029, 001035, 002548
            skus_problema = ['001027', '001029', '001035', '002548']
            skus_problema_encontrados = []
            soma_skus_problema = 0.0
            for idx, row in df_totais_vol_recalc.iterrows():
                sku = str(row.get('sku', ''))
                if sku in skus_problema:
                    saldo_val = float(row.get('saldo', 0.0))
                    skus_problema_encontrados.append({
                        'sku': sku,
                        'produto': str(row.get('produto', '')),
                        'saldo': saldo_val
                    })
                    soma_skus_problema += saldo_val

            if skus_problema_encontrados:
                st.warning(f"⚠️ ATENÇÃO: {len(skus_problema_encontrados)} SKUs podem estar causando diferença no cálculo (soma: {formatar_br(soma_skus_problema)}):")
                for sku_prob in skus_problema_encontrados:
                    st.caption(f"  - SKU {sku_prob['sku']}: {sku_prob['produto']} - Saldo: {formatar_br(sku_prob['saldo'])}")

        vol_total = vol_total_recalc
        total_saldo = vol_total
        # Atualizar df_totais_vol e df_totais_calc para garantir consistência
        df_totais_vol = df_totais_vol_recalc
        df_totais_calc = df_totais_vol_recalc
    ...
    # IMPORTANTE: df_para_exibicao é uma cópia do df_view que JÁ está filtrado por categoria
    df_para_exibicao = df_view.copy()

    # CORREÇÃO CRÍTICA: Garantir que vol_total e total_saldo sejam sempre iguais
    # Se dados vêm do HTML, ambos já foram calculados acima e são iguais
    # Se dados vêm do histórico, calcular total_saldo do df_para_exibicao e atualizar ambos
    if not dados_do_html:
        # CORREÇÃO CRÍTICA: Para histórico, usar a MESMA LÓGICA do HTML
        # O total já foi calculado acima diretamente do df_full original (linha 1163-1185)
        # Apenas garantir que total_saldo = vol_total (já estão iguais acima)
        # Não precisa recalcular, pois já foi calculado corretamente acima usando df_full original
        pass

    # CORREÇÃO: Atualizar a métrica exibida usando st.empty() e rerun, ou simplesmente garantir que vol_total = total_saldo
    # Como Streamlit não permite atualizar métricas dinamicamente, garantimos que ambos sejam iguais
    # Para dados do HTML, já estão iguais. Para histórico, vol_total foi atualizado acima.
    ...
    if 'saldo' in df_para_exibicao.columns and not df_para_exibicao.empty and 'sku' in df_para_exibicao.columns:
        ...
        # CORREÇÃO: total_saldo já foi calculado acima (linha ~1176) antes de modificar df_para_exibicao
        # Usar o valor já calculado para garantir consistência com vol_total
        # Não recalcular aqui para evitar diferenças
        ...
        # Calcular totais de entrada e saída APENAS dos SKUs que estão visíveis
        # CORREÇÃO: Usar os mesmos SKUs que foram usados no cálculo do total_saldo
        total_entrada = 0.0
        total_saida = 0.0

        # Pegar SKUs únicos do df_para_exibicao (os que estão visíveis)
        skus_visiveis = set(df_para_exibicao['sku'].dropna().unique()) if 'sku' in df_para_exibicao.columns else set()

        if not df_totais.empty and skus_visiveis:
            # Filtrar linhas TOTAL apenas dos SKUs visíveis
            df_totais_filtrado = df_totais[df_totais['sku'].isin(skus_visiveis)]
        ...
        # Se não tem linhas TOTAL suficientes, somar das movimentações dos SKUs visíveis
        if total_entrada == 0.0 and total_saida == 0.0 and not df_movimentacoes.empty and skus_visiveis:
            df_mov_filtrado = df_movimentacoes[df_movimentacoes['sku'].isin(skus_visiveis)]
        ...
        for col in df_para_exibicao.columns:
            ...
            elif col == 'saldo_anterior':
                # Somar saldo_anterior apenas das linhas TOTAL (ou última movimentação de cada SKU)
                if col in df_calc_total.columns:
                    # Usar os mesmos SKUs que foram usados no cálculo do total_saldo
                    if 'tipo' in df_calc_total.columns:
                        mask_total_saldo_ant = df_calc_total['tipo'].astype(str).str.strip().str.upper().isin(['TOTAL', 'TOTAL:', 'TOTAL '])
                        df_totais_saldo_ant = df_calc_total[mask_total_saldo_ant].copy()
                        if not df_totais_saldo_ant.empty:
                            # Remover duplicatas por SKU (uma linha TOTAL por SKU)
                            if 'sku' in df_totais_saldo_ant.columns:
                                df_totais_saldo_ant = df_totais_saldo_ant.drop_duplicates(subset=['sku'], keep='last')
                            linha_total[col] = pd.to_numeric(df_totais_saldo_ant[col], errors='coerce').fillna(0.0).sum()
                        else:
                            linha_total[col] = 0.0
                    else:
                    ...
                    linha_total[col] = 0.0
            elif col in ['vlr_unit', 'vlr_total']:
                # CORREÇÃO CRÍTICA: Somar vlr_unit e vlr_total APENAS das linhas TOTAL de cada SKU
                # Não somar de todas as linhas (movimentações), apenas das linhas TOTAL
                if col in df_calc_total.columns:
                    if 'tipo' in df_calc_total.columns:
                        # Filtrar apenas linhas TOTAL
                        mask_total_vlr = df_calc_total['tipo'].astype(str).str.strip().str.upper().isin(['TOTAL', 'TOTAL:', 'TOTAL '])
                        df_totais_vlr = df_calc_total[mask_total_vlr].copy()
                        if not df_totais_vlr.empty:
                            # Remover duplicatas por SKU (uma linha TOTAL por SKU)
                            if 'sku' in df_totais_vlr.columns:
                                df_totais_vlr = df_totais_vlr.drop_duplicates(subset=['sku'], keep='last')
                            linha_total[col] = pd.to_numeric(df_totais_vlr[col], errors='coerce').fillna(0.0).sum()
                        else:
                            linha_total[col] = 0.0
                    else:
                        # Se não tem tipo, somar todas (fallback)
                        linha_total[col] = pd.to_numeric(df_calc_total[col], errors='coerce').fillna(0.0).sum()
                else:
                    linha_total[col] = 0.0
            else:
    ...
    if allow_save:
        ...
        if st.button("💾 Salvar Filtrados no Banco (v191)", key=f"{key_prefix}_btn_save"):
            # Prepare for DB
            # CORREÇÃO CRÍTICA: Salvar df_full (dados completos) ao invés de df_view (filtrado)
            # Isso garante que todas as linhas TOTAL sejam salvas, não apenas as visíveis após filtros
            ...
            # V193: Incluir vlr_unit e vlr_total nas colunas a serem salvas
            cols_db_plenus = ['sku', 'produto', 'categoria', 'data_movimento', 'tipo_movimento',
                              'entrada', 'saida', 'saldo_apos', 'saldo_anterior', 'nota', 'serie', 'vlr_unit', 'vlr_total', 'arquivo_origem']
            ...
            # V194: Verificar se o arquivo já foi importado antes de salvar
            # O banco deve ser o espelho do HTML importado, não permitir importar duas vezes
            arquivo_origem = None
            if 'arquivo_origem' in df_save.columns and not df_save.empty:
                arquivo_origem = df_save['arquivo_origem'].iloc[0]
            ...
            try:
                if arquivo_origem:
                    query_check = "SELECT COUNT(*) as qtd FROM plenus_historico WHERE arquivo_origem = ?"
                    df_check = pd.read_sql(query_check, conn_check, params=(arquivo_origem,))
                    qtd_existente = df_check.iloc[0]['qtd'] if not df_check.empty else 0

                    if qtd_existente > 0:
                        st.warning(f"⚠️ Este arquivo já foi importado anteriormente: {arquivo_origem}")
                        st.info(f"📊 O banco já contém {qtd_existente} registros deste arquivo.")
                        st.info("💡 Para reimportar, exclua os dados do período primeiro ou exclua por arquivo.")
                        conn_check.close()
                        return  # Não salvar se já foi importado
            ...
            # CORREÇÃO v194: Se o arquivo não foi importado, salvar normalmente
